// Tests for `src/codebase/profiles/ecosystems/cargo.rs`.
//
...
#[test]
fn cargo_is_registered_with_its_manifest_and_lockfiles() {
    let profile = ecosystem_profile("cargo").expect("cargo is registered");
    assert_eq!(profile.id, "cargo");
    assert_eq!(profile.manifest, Some("Cargo.toml"));
    assert_eq!(profile.lockfiles, ["Cargo.lock"]);
}
