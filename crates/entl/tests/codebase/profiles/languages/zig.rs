// Tests for `src/codebase/profiles/languages/zig.rs`.
//
...
#[test]
fn zig_is_registered_under_its_id() {
    let profile = language_profile("zig").expect("zig is registered");
    assert_eq!(profile.id, "zig");
    assert!(!profile.display_name.is_empty());
}

#[test]
fn zig_claims_its_extensions() {
    let profile = language_profile("zig").expect("zig is registered");
    assert_eq!(profile.extensions, ["zig"]);
}
