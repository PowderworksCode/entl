// Tests for `src/codebase/profiles/languages/php.rs`.
//
...
#[test]
fn php_is_registered_under_its_id() {
    let profile = language_profile("php").expect("php is registered");
    assert_eq!(profile.id, "php");
    assert!(!profile.display_name.is_empty());
}

#[test]
fn php_claims_its_extensions() {
    let profile = language_profile("php").expect("php is registered");
    assert_eq!(profile.extensions, ["php"]);
}
