// Tests for `src/codebase/profiles/languages/sql.rs`.
//
...
#[test]
fn sql_is_registered_under_its_id() {
    let profile = language_profile("sql").expect("sql is registered");
    assert_eq!(profile.id, "sql");
    assert!(!profile.display_name.is_empty());
}

#[test]
fn sql_claims_its_extensions() {
    let profile = language_profile("sql").expect("sql is registered");
    assert_eq!(profile.extensions, ["sql"]);
}
