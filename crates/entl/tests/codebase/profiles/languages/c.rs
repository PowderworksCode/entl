// Tests for `src/codebase/profiles/languages/c.rs`.
//
...
#[test]
fn c_is_registered_under_its_id() {
    let profile = language_profile("c").expect("c is registered");
    assert_eq!(profile.id, "c");
    assert!(!profile.display_name.is_empty());
}

#[test]
fn c_claims_its_extensions() {
    let profile = language_profile("c").expect("c is registered");
    assert_eq!(profile.extensions, ["c", "h"]);
}
