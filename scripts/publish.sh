...
#
# Adapted from the fleet's conf (.ordnung/managed/publishing/rust/publish.sh).
# That copy publishes a crate at the repository root; entl's crate is a
# workspace member, so the cargo invocation below names it with -p.
#
