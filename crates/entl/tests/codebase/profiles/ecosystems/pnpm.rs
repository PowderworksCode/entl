// Tests for `src/codebase/profiles/ecosystems/pnpm.rs`.
//
...
#[test]
fn pnpm_is_registered_with_its_manifest_and_lockfiles() {
    let profile = ecosystem_profile("pnpm").expect("pnpm is registered");
    assert_eq!(profile.id, "pnpm");
    assert_eq!(profile.manifest, Some("package.json"));
    assert_eq!(profile.lockfiles, ["pnpm-lock.yaml"]);
}
