// Tests for `src/codebase/profiles/languages/toml.rs`.
//
...
#[test]
fn toml_is_registered_under_its_id() {
    let profile = language_profile("toml").expect("toml is registered");
    assert_eq!(profile.id, "toml");
    assert!(!profile.display_name.is_empty());
}

#[test]
fn toml_claims_its_extensions() {
    let profile = language_profile("toml").expect("toml is registered");
    assert_eq!(profile.extensions, ["toml"]);
}
