; Discarded-error recognition for JavaScript.
;
; The typescript pack's file, copied unchanged: `catch_clause`, `void` and the
; `.catch(..)` member call are all plain ECMAScript and nothing here names a
; TypeScript-only node kind. The two languages spell these forms identically
; because they ARE the same forms -- what TypeScript adds is annotation, and no
; form below reads one.
;
