// Tests for `src/codebase/profiles/languages/css.rs`.
//
...
#[test]
fn css_is_registered_under_its_id() {
    let profile = language_profile("css").expect("css is registered");
    assert_eq!(profile.id, "css");
    assert!(!profile.display_name.is_empty());
}

#[test]
fn css_claims_its_extensions() {
    let profile = language_profile("css").expect("css is registered");
    assert_eq!(profile.extensions, ["css"]);
}
