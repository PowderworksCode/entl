...
Rust APIs. The first crate, `entl`, turns a source tree into facts
...
```sh
cargo run -p entl --example inspect -- PATH
```

`crates/entl/src/codebase/profiles/verbosity.rs` and `docs/verbosity-*.md` are
generated by `tools/verbosity` from a corpus checkout. Edit the tool and
regenerate; do not hand-edit either output. See its README before changing what
gets measured — one of the corpora is GFDL 1.2 and none of its content may enter
this repository. [notes/verbosity.md](notes/verbosity.md) explains what the
numbers mean and which corpora were already tried.
