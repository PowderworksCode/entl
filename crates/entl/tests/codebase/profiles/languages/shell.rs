// Tests for `src/codebase/profiles/languages/shell.rs`.
//
...
#[test]
fn shell_is_registered_under_its_id() {
    let profile = language_profile("shell").expect("shell is registered");
    assert_eq!(profile.id, "shell");
    assert!(!profile.display_name.is_empty());
}

#[test]
fn shell_claims_its_extensions() {
    let profile = language_profile("shell").expect("shell is registered");
    assert_eq!(profile.extensions, ["sh", "bash", "zsh", "fish"]);
}
